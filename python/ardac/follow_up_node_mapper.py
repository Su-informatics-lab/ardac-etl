...
def generate_observational_follow_up_node(
    ...
    logger.info(
        "Reading the observational liver scores file: %s", obs_liver_scores_path.as_posix()
    )
    df_obs_liver_scores_input = pd.read_csv(
        obs_liver_scores_path.as_posix(), sep=",", dtype=str
    )
    logger.info("Done reading observational liver scores file")

    logger.info(
        "Reading the observational medical information file: %s", obs_med_info_path.as_posix()
    )
    df_obs_med_info_input = pd.read_csv(
        obs_med_info_path.as_posix(), sep=",", dtype=str
    )
    logger.info("Done reading observational medical information file")

    logger.info("Reading the observational vitals file: %s", obs_vitals_path.as_posix())
    ...
    for _, row in case_table.iterrows():
        ...
        for week, ext in extensions.items():
            ...
            df_output = pd.concat(
                [df_output, pd.DataFrame([new_row])], ignore_index=True
            )
    ...
    for _, row in df_obs_liver_scores_input.iterrows():
        ...
        if not output_row_index.empty:
            ...
            df_output.loc[output_row_index, "tlfb_drinking_days"] = row.get(
                "tlfbnumdd", None
            )
            df_output.loc[output_row_index, "tlfb_number_drinks"] = row.get(
                "tlfbnumd", None
            )
            df_output.loc[output_row_index, "liver_score_date"] = row.get(
                "liverdat", None
            )
    ...
    for _, row in df_obs_med_info_input.iterrows():
        ...
        if not output_row_index.empty:
            ...
            df_output.loc[output_row_index, "liver_transplant"] = row.get(
                "livtnsplyn", None
            )
            df_output.loc[output_row_index, "ascites_date"] = row.get("ascdat", None)
            df_output.loc[output_row_index, "hep_enceph_diagnosis_date"] = row.get(
                "hependat", None
            )
            df_output.loc[output_row_index, "varices_diagnosis_date"] = row.get(
                "vardat", None
            )
            df_output.loc[output_row_index, "hepcar_diagnosis_date"] = row.get(
                "hepcardat", None
            )
            df_output.loc[output_row_index, "liver_transplant_date"] = row.get(
                "livtnspldat", None
            )
    ...
    for _, row in df_obs_soc_input.iterrows():
        ...
        if not output_row_index.empty:
            # Perform the mapping
            output_row_index = output_row_index[0]  # Get the first matching index
            df_output.loc[output_row_index, "infection_screen_done"] = row.get(
                "infscreennd", None
            )
            df_output.loc[output_row_index, "infection_screen_date"] = row.get(
                "infscreen_date", None
            )
            df_output.loc[output_row_index, "blood_culture"] = row.get(
                "socisbcnd___999", None
            )
            df_output.loc[output_row_index, "blood_culture_result"] = row.get(
                "socisbc", None
            )
            df_output.loc[output_row_index, "blood_organism"] = row.get(
                "socisbc_pos", None
            )
            df_output.loc[output_row_index, "blood_culture_date"] = row.get(
                "socisbcdat", None
            )
            df_output.loc[output_row_index, "urine_culture"] = row.get(
                "socisucnd___999", None
            )
            df_output.loc[output_row_index, "urine_culture_result"] = row.get(
                "socisuc", None
            )
            df_output.loc[output_row_index, "urine_culture_organism"] = row.get(
                "socisuc_pos", None
            )
            df_output.loc[output_row_index, "urine_culture_date"] = row.get(
                "socisucdat", None
            )
            df_output.loc[output_row_index, "urine_culture_fungal_result"] = row.get(
                "soicuc_fung", None
            )
            df_output.loc[output_row_index, "ascites_culture"] = row.get(
                "socisacnd___999", None
            )
            df_output.loc[output_row_index, "ascites_culture_result"] = row.get(
                "socisac", None
            )
            df_output.loc[output_row_index, "ascites_organism"] = row.get(
                "socisac_pos", None
            )
            df_output.loc[output_row_index, "ascites_date"] = row.get(
                "socisacdat", None
            )
            ...
            df_output.loc[output_row_index, "esophageal_varices_size"] = row.get(
                "endovarsiz_esoph", None
            )
            df_output.loc[output_row_index, "esophageal_varices_bleed"] = row.get(
                "endobled_esoph", None
            )
            df_output.loc[output_row_index, "gastric_varices_size"] = row.get(
                "endovarsiz_gast", None
            )
            df_output.loc[output_row_index, "gastric_varices_bleed"] = row.get(
                "endobled_gast", None
            )
            df_output.loc[output_row_index, "portal_hypertensive_gastropathy"] = (
                row.get("porthypsev", None)
            )
            df_output.loc[output_row_index, "esophageal_ulcer_size"] = row.get(
                "endoulcsiz_esoph", None
            )
            df_output.loc[output_row_index, "esophageal_ulcer_bleed"] = row.get(
                "endoulcbled_esoph", None
            )
            df_output.loc[output_row_index, "gastric_ulcer_size"] = row.get(
                "endoulcsiz_gast", None
            )
            df_output.loc[output_row_index, "gastric_ulcer_bleed"] = row.get(
                "endoulcbled_gast", None
            )
            df_output.loc[output_row_index, "duodenum_ulcer_size"] = row.get(
                "endoulcsiz_duod", None
            )
            df_output.loc[output_row_index, "duodenum_ulcer_bleed"] = row.get(
    ...
    non_empty_columns = list(
        set(df_output.columns)
        - {
            "*type",
            "project_id",
            "*submitter_id",
            "cases.submitter_id",
            "demographics.submitter_id",
            "*days_to_follow_up",
            "visit_day",
        }
    )
    ...
    for index, row in df_output.iterrows():
        ...
        if row[non_empty_columns].isnull().all():
            ...
            qc_records.append(
                {
                    "usubjid": row["cases.submitter_id"].split("_")[
                        0
                    ],  # Extract usubjid
                    "*submitter_id": row["*submitter_id"],
                    "empty_follow-up": "Y",
...
def generate_clinical_follow_up_node(
    rct_liver_scores_path: Path,
    rct_med_info_path: Path,
    rct_vitals_path: Path,
    rct_soc_path: Path,
    rct_case_path: Path,
    template_headers: list[str],
) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Read the file using pandas
    logger.info(
        "Reading the clinical liver scores file: %s", rct_liver_scores_path.as_posix()
    )
    df_rct_liver_scores_input = pd.read_csv(
        rct_liver_scores_path.as_posix(), sep=",", dtype=str
    )
    logger.info("Done reading clinical liver scores file")

    logger.info(
        "Reading the clinical medical information file: %s", rct_med_info_path.as_posix()
    )
    df_rct_med_info_input = pd.read_csv(
        rct_med_info_path.as_posix(), sep=",", dtype=str
    )
    logger.info("Done reading clinical medical information file")

    logger.info("Reading the clinical vitals file: %s", rct_vitals_path.as_posix())
    ...
    logger.info("Done reading clinical case file")

    # Extract "*submitter_id" from df_obs_case and create case_table
    case_table_rct = pd.DataFrame()
    case_table_rct["*submitter_id"] = df_rct_case_input["*submitter_id"]
    case_table_rct["usubjid"] = case_table_rct["*submitter_id"].apply(
        lambda x: x.split("_")[0]
    )  # Extract the number before "_"
    ...
    for _, row in case_table_rct.iterrows():
        ...
        for day, ext in extensions_rct.items():
            ...
            df_output_rct = pd.concat(
                [df_output_rct, pd.DataFrame([new_row])], ignore_index=True
            )
    ...
    for _, row in df_rct_liver_scores_input.iterrows():
        ...
        # Find the matching row in df_output
        output_row_index = df_output_rct[
            df_output_rct["*submitter_id"] == submitter_id
        ].index
        if not output_row_index.empty:
            # Perform the mapping
            output_row_index = output_row_index[0]  # Get the first matching index
            df_output_rct.loc[output_row_index, "meld_score"] = row.get("meld", None)
            df_output_rct.loc[output_row_index, "child_pugh_score"] = row.get(
                "cps", None
            )
            df_output_rct.loc[output_row_index, "tlfb_drinking_days"] = row.get(
                "tlfbnumdd", None
            )
            df_output_rct.loc[output_row_index, "tlfb_number_drinks"] = row.get(
                "tlfbnumd", None
            )
    ...
    for _, row in df_rct_soc_input.iterrows():
        ...
        if not output_row_index.empty:
            # Perform the mapping
            output_row_index = output_row_index[0]  # Get the first matching index
            df_output_rct.loc[output_row_index, "infection_screen_done"] = row.get(
                "infscreennd", None
            )
            df_output_rct.loc[output_row_index, "infection_screen_date"] = row.get(
                "infscreen_date", None
            )
            df_output_rct.loc[output_row_index, "blood_culture"] = row.get(
                "socisbcnd___999", None
            )
            df_output_rct.loc[output_row_index, "blood_culture_result"] = row.get(
                "socisbc", None
            )
            df_output_rct.loc[output_row_index, "blood_organism"] = row.get(
                "socisbc_pos", None
            )
            df_output_rct.loc[output_row_index, "blood_culture_date"] = row.get(
                "socisbcdat", None
            )
            df_output_rct.loc[output_row_index, "urine_culture"] = row.get(
                "socisucnd___999", None
            )
            df_output_rct.loc[output_row_index, "urine_culture_result"] = row.get(
                "socisuc", None
            )
            df_output_rct.loc[output_row_index, "urine_culture_organism"] = row.get(
                "socisuc_pos", None
            )
            df_output_rct.loc[output_row_index, "urine_culture_date"] = row.get(
                "socisucdat", None
            )
            df_output_rct.loc[output_row_index, "urine_culture_fungal_result"] = (
                row.get("soicuc_fung", None)
            )
            df_output_rct.loc[output_row_index, "ascites_culture"] = row.get(
                "socisacnd___999", None
            )
            df_output_rct.loc[output_row_index, "ascites_culture_result"] = row.get(
                "socisac", None
            )
            df_output_rct.loc[output_row_index, "ascites_organism"] = row.get(
                "socisac_pos", None
            )
            df_output_rct.loc[output_row_index, "ascites_date"] = row.get(
                "socisacdat", None
            )
            df_output_rct.loc[output_row_index, "endoscopy"] = row.get("endond", None)
            df_output_rct.loc[output_row_index, "endoscopy_date"] = row.get(
                "endodat", None
            )
            df_output_rct.loc[output_row_index, "esophageal_varices_size"] = row.get(
                "endovarsiz_esoph", None
            )
            df_output_rct.loc[output_row_index, "esophageal_varices_bleed"] = row.get(
                "endobled_esoph", None
            )
            df_output_rct.loc[output_row_index, "gastric_varices_size"] = row.get(
                "endovarsiz_gast", None
            )
            df_output_rct.loc[output_row_index, "gastric_varices_bleed"] = row.get(
                "endobled_gast", None
            )
            df_output_rct.loc[output_row_index, "portal_hypertensive_gastropathy"] = (
                row.get("porthypsev", None)
            )
            df_output_rct.loc[output_row_index, "esophageal_ulcer_size"] = row.get(
                "endoulcsiz_esoph", None
            )
            df_output_rct.loc[output_row_index, "esophageal_ulcer_bleed"] = row.get(
                "endoulcbled_esoph", None
            )
            df_output_rct.loc[output_row_index, "gastric_ulcer_size"] = row.get(
                "endoulcsiz_gast", None
            )
            df_output_rct.loc[output_row_index, "gastric_ulcer_bleed"] = row.get(
                "endoulcbled_gast", None
            )
            df_output_rct.loc[output_row_index, "duodenum_ulcer_size"] = row.get(
                "endoulcsiz_duod", None
            )
            df_output_rct.loc[output_row_index, "duodenum_ulcer_bleed"] = row.get(
                "endoulcbled_duod", None
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dcc_liver_scores_file",
        dest="dccLiverScoresFile",
        required=True,
        help="Full path to the DCC input liver scores file in CSV format",
    )
    parser.add_argument(
        "--dcc_med_info_file",
        dest="dccMedInfoFile",
        required=True,
        help="Full path to the DCC input medical information file in CSV format",
    )
    parser.add_argument(
        "--dcc_vitals_file",
        dest="dccVitalsFile",
        required=True,
        help="Full path to the DCC input vitals file in CSV format",
    )
    parser.add_argument(
        "--dcc_soc_file",
        dest="dccSOCFile",
