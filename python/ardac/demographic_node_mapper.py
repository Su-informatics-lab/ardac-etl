...
def generate_observational_demographic_node(
    obs_subjects_path: Path, obs_case_path: Path, template_headers: list[str]
) -> pd.DataFrame:
    """
    ...
    logger.info(
        "Reading the observational subjects file: %s", obs_subjects_path.as_posix()
    )
    df_obs_subjects_input = pd.read_csv(
        obs_subjects_path.as_posix(), sep=",", dtype=str
    )
    ...
    df_obs_output = pd.DataFrame(
        index=df_obs_subjects_input.index, columns=template_headers
    )
    ...
    for _, row in case_table.iterrows():
        ...
        if not input_row.empty:
            ...
            df_obs_output.loc[_, "cause_of_death_secondary"] = input_row.get(
                "cods", None
            )
            ...
            df_obs_output.loc[_, "year_of_birth"] = (
                brthdtc.split("-")[0] if pd.notna(brthdtc) else None
            )

            dthdtc = input_row.get("dthdtc", None)
            df_obs_output.loc[_, "year_of_death"] = (
                dthdtc.split("-")[0] if pd.notna(dthdtc) else None
            )
...
def generate_clinical_demographic_node(
    rct_subjects_path: Path, rct_case_path: Path, template_headers: list[str]
) -> pd.DataFrame:
    """
    ...
    """
    # Read the file using pandas
    logger.info("Reading the clinical subjects file: %s", rct_subjects_path.as_posix())
    df_rct_subjects_input = pd.read_csv(
        rct_subjects_path.as_posix(), sep=",", dtype=str
    )
    ...
    df_rct_output = pd.DataFrame(
        index=df_rct_subjects_input.index, columns=template_headers
    )
    ...
    for _, row in case_table.iterrows():
        ...
        if not input_row.empty:
            ...
            df_rct_output.loc[_, "cause_of_death_secondary"] = input_row.get(
                "cods", None
            )
            ...
            # Extract "year_of_birth" and "year_of_death"
            brthdtc = input_row.get("brthdtc", None)
            df_rct_output.loc[_, "year_of_birth"] = (
                brthdtc.split("-")[0] if pd.notna(brthdtc) else None
            )

            dthdtc = input_row.get("dthdtc", None)
            df_rct_output.loc[_, "year_of_death"] = (
                dthdtc.split("-")[0] if pd.notna(dthdtc) else None
            )
...
def main(command_arguments: argparse.Namespace) -> int:
    ...
    if not template_path.is_file():
        logger.critical(
            "Cannot find demographic template file: %s", template_path.as_posix()
        )
    ...
    logger.info("Reading demographic template CSV file: %s", template_path.as_posix())
    ...
    if command_arguments.subjectsType == "observational":
        ...
        logger.info("Clinical demographic node saved as: %s", node_file_path.as_posix())
    else:
        raise ValueError(
            f"Processing for subjects_type={command_arguments.subjectsType} is not implemented"
        )
