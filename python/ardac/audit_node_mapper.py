...
def generate_observational_audit_node(
    ...
    logger.info("Reading the observational audit file: %s", obs_audit_path.as_posix())
    df_obs_audit_input = pd.read_csv(obs_audit_path.as_posix(), sep=",", dtype=str)
    logger.info("Done reading observational audit file")

    logger.info("Reading the observational ARDaC case file: %s", obs_case_path.as_posix())
    ...
    for _, row in case_table.iterrows():
        ...
        if not input_row.empty:
            ...
            unmatched_records_obs.append({"usubjid": usubjid, "*submitter_id": submitter_id, "missing_audit": "Y"})
...
def generate_clinical_audit_node(
    rct_audit_path: Path, rct_case_path: Path, template_headers: list[str]
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    """
    # Read the file using pandas
    logger.info("Reading the clinical audit file: %s", rct_audit_path.as_posix())
    df_rct_audit_input = pd.read_csv(rct_audit_path.as_posix(), sep=",", dtype=str)
    logger.info("Done reading clinical audit file")

    logger.info("Reading the observational ARDaC case file: %s", rct_case_path.as_posix())
    ...
    for _, row in case_table.iterrows():
        ...
        if not input_row.empty:
            ...
            # Add unmatched record to the list
            unmatched_records_rct.append({"usubjid": usubjid, "*submitter_id": submitter_id, "missing_audit": "Y"})
            # Remove the unmatched row from df_rct_output
...
def main(command_arguments: argparse.Namespace) -> int:
    ...
    logger.info("Reading audit template CSV file: %s", template_path.as_posix())
    ...
    if command_arguments.subjectsType == "observational":
        ...
        logger.info("Observational audit node saved as: %s", node_file_path.as_posix())
        ...
        logger.info("Clinical QC file saved as: %s", node_file_unmatched_path.as_posix())
    else:
        raise ValueError(f"Processing for subjects_type={command_arguments.subjectsType} is not implemented")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="dccAuditFile",
        required=True,
        help="Full path to the DCC input audit file in CSV format",
    )
    parser.add_argument(
        "--node_output_path",
        dest="nodeOutputPath",
