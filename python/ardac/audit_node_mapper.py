...
def generate_observational_audit_node(obs_audit_path: Path, obs_case_path: Path, template_headers: list[str]) -> pd.DataFrame:
   ...
   ----------
   obs_audit_path : Path
      The full path to the audit file for the DCC observational data
   obs_case_path : Path
...
def main(command_arguments: argparse.Namespace, logger: logging.Logger) -> int:
   ...
   if command_arguments.subjectsType == 'observational':
      ...
      df_unmatched_rct.to_csv(node_file_unmatched_path, sep='\t', index=False)
...
if __name__ == '__main__':
   parser = argparse.ArgumentParser(
      description='''This utility generates ARDaC audit nodes from observational or clinical trial DCC audit files and ARDaC case node files.
         ...
         and the path to where the ARDaC audit node file is to be written.''',
      epilog=f'''Observational audit node files are named \'{_constants.audit_obs_file_name}\'.
         Clinical audit node files are named \'{_constants.audit_rct_file_name}\'.  Audit node files are written to the directory given by the --node_output_path argument.
